...
import com.ane.bean.LoadCarDTO
...
/**
 * @ClassName ActualLoadDataToHiveJob
 * @Description actual_load_data_queue装车数据写入hive
 * @Author zhangleilei
 * @Date 2020/9/15 15:15
 * @Version 1.0
 */

object ActualLoadDataToHiveJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    properties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "ActualLoadDataToHiveJob")
    ...
    val flinkKafkaConsumer = new FlinkKafkaConsumer[String](ConfigUtils.kafkaTopicActualLoadData, new SimpleStringSchema(), properties)
...
//        println("kafka data ==============================" + value)
        val record = JSON.parseObject(value, classOf[LoadCarDTO])
//        println("record data ==============================" + record)
    ...
    tableEnv.createTemporaryView("tmp_actual_load", dataTable)


    val insertSql = """
                      |INSERT INTO t_actual_load
                      |select
                      |id as id,
