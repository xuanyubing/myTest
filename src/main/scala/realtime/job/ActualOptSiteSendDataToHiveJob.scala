...
import com.ane.bean.OperaSiteSendDto
...
/**
 * @ClassName ActualOptSiteSendDataToHiveJob
 * @Description actual_opt_site_send_data_queue 网点发件数据写入hive
 * @Author zhangleilei
 * @Date 2020/9/15 15:15
 * @Version 1.0
 */

object ActualOptSiteSendDataToHiveJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    properties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "ActualOptSiteSendDataToHiveJob")
    ...
    val flinkKafkaConsumer = new FlinkKafkaConsumer[String](ConfigUtils.kafkaTopicActualOptSiteSendData, new SimpleStringSchema(), properties)
...
//        println("kafka data ==============================" + value)
        val record = JSON.parseObject(value, classOf[OperaSiteSendDto])
//        println("record data ==============================" + record)
    ...
    tableEnv.createTemporaryView("tmp_actual_opt_site_send", dataTable)


    val insertSql = """
                      |INSERT INTO t_actual_opt_site_send
                      |select
                      |siteSendId as site_send_id,
