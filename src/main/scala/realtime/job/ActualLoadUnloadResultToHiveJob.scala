...
/**
 * @ClassName ActualLoadUnloadResultToHiveJob
 * @Description actual_load_unload_queue02 实际数据写入hive
 * @Author zhangleilei
 * @Date 2020/9/15 15:15
 * @Version 1.0
 */

object ActualLoadUnloadResultToHiveJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    properties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "ActualLoadUnloadResultToHiveJob")
...
//    properties.put("flink.partition-discovery.interval-millis","10000")
    ...
    val flinkKafkaConsumer = new FlinkKafkaConsumer[String](ConfigUtils.kafkaTopicActualLoadUnload, new SimpleStringSchema(), properties)
...
//        println("record data ==============================" + record)
    ...
    tableEnv.createTemporaryView("tmp_actual_load_unload_result", dataTable)


    val insertSql = """
                      |INSERT INTO t_actual_load_unload_result
                      |select
                      |version as version,
                      ...
                      |from tmp_actual_load_unload_result
                      |""".stripMargin
