...
/**
 * @ClassName ActualForecastResultToHiveJob
 * @Description actual_forecast_result_queue 预报数据写入hive
 * @Author zhangleilei
 * @Date 2020/9/15 15:15
 * @Version 1.0
 */

object ActualForecastResultToHiveJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    properties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "ActualForecastResultToHiveJob")
...
//    properties.put("flink.partition-discovery.interval-millis","10000")
    ...
    val flinkKafkaConsumer = new FlinkKafkaConsumer[String](ConfigUtils.kafkaTopicActualForecastResult, new SimpleStringSchema(), properties)
    ...
    tableEnv.createTemporaryView("tmp_actual_forecast_result", dataTable)


    val insertSql = """
                      |INSERT INTO t_actual_forecast_result
                      |select
                      |version as version,
                      ...
                      |from tmp_actual_forecast_result
                      |""".stripMargin
